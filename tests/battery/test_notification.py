...
def battery_notification_view():
    view = BatteryNotificationView(nag_lobound=10)

    view.notifications = Mock()

    return view


def test_notifies_when_discharging_and_low(battery_notification_view):
    battery_notification_view.render({"data": BatteryData(9, BatteryState.DISCHARGING)})
...
def test_does_not_notify_when_discharging_and_high(battery_notification_view):
    battery_notification_view.render(
        {"data": BatteryData(11, BatteryState.DISCHARGING)}
    )
...
def test_closes_notification_when_charging_and_low(battery_notification_view):
    ...
    battery_notification_view.render({"data": BatteryData(9, BatteryState.CHARGING)})
